<template>
  <header class="header">
    ...
    <div class="navbar">
      ...
      <div class="navbar-right-container" style="display: flex;">
        <div class="navbar-menu-container">
          ...
          <a href="javascript:void(0)" class="navbar-link" v-if="!nickName" @click="showModal">Login</a>
    ...
    <!-- 登录模态框 -->
    <div id="loginModal" class="md-modal modal-msg md-modal-transition" v-bind:class="{'md-show':loginModalFlag}">
...
export default {
  ...
  computed: {
    ...mapState(["nickName", "cartCount", "loginModalFlag"])
    // nickName() {
  ...
  methods: {
      ...
      axios
        ...
        .then(res => {
          ...
          if (r.status === "0") {
            ...
            this.$store.commit("updateUserInfo", r.result.userName);
            this.getCartCount();
    ...
    checkLogin() {
      axios.post("/users/checkLogin").then(res => {
        ...
        if (r.status === "0") {
          ...
          this.$store.commit("updateUserInfo", r.result);
          this.getCartCount();
          // this.loginModalFlag = false;
          this.$store.commit("loginState", false);
    ...
    getCartCount() {
      axios.get("/users/getCartCount").then(res => {
        let r = res.data;
        if (r.status === "0") {
          this.$store.commit("initCartCount", r.result);
        }
      });
    },
    // 关闭登录框
